...
def measure(path: Path) -> tuple[int, int, int, int, int]:
    reset_scene()
    ...
    return len(meshes), vertices, triangles, len(materials), len(images)


def main() -> None:
    paths = sorted((ASSET_ROOT / "hero").glob("*.glb"))
    paths += sorted((ASSET_ROOT / "kaykit-dungeon").glob("*.gltf"))
    print("asset,bytes,mesh_objects,vertices,triangles,materials,file_images")
    for path in paths:
        mesh_objects, vertices, triangles, materials, images = measure(path)
        print(
